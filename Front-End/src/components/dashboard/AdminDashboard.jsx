...
import {faChartBar ,faHotel ,faCalendarCheck ,faUser ,faSignOut ,faCreditCard, faUsers,
  faUserTie,
  faChild,
  faCheckCircle,
        ...
        <aside>
           <ul className="absolute top-0 left-0 bg-blue-400 h-screen p-12">
            {sideBarElement.map((element , index)=>(
              ...
              className={`p-2 cursor-pointer rounded-lg ${activeTab==element.name ? "bg-blue-900 text-white font-semibold" : ""}`}>
                <h1 className= ""> <span className="pr-3"><FontAwesomeIcon icon={element.icon} className={` ${activeTab==element.name ? "text-white" :"text-blue-900"}`}/></span> {element.name} </h1>
              </li>
            ))}
           </ul>
        </aside>
      ...
      <div>
        <h1 className="text-xl font-semibold text-orange-500 mt-4">
          ...
          <div
            ...
            className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 mt-9 "
          >
            <div className="bg-gray-400 text-center p-8 w-full rounded-xl">
              ...
              <h2 className="text-2xl text-white"> {setClientStatistic.clientNumber} </h2>
            </div>
            <div className="bg-yellow-400 text-center p-8 w-full rounded-xl">
              ...
              <h2 className="text-2xl text-white">{setClientStatistic.adultNumber}</h2>
            </div>
            <div className="bg-blue-400 text-center p-8 w-full rounded-xl">
              ...
              <h2 className="text-2xl text-white">{setClientStatistic.childrenNumber}</h2>
            </div>
            <div className="bg-green-400 text-center p-8 w-full rounded-xl">
              ...
              <h1 className="font-bold text-2xl text-white">success Reservation</h1>
              <h2 className="text-2xl text-white">{setClientStatistic.successReservation}</h2>
            </div>
            <div className="bg-red-400 text-center p-8 w-full rounded-xl">
              <FontAwesomeIcon icon={faTrashAlt} className="text-2xl text-white"/>
              <h1 className="font-bold text-2xl text-white">deleted Reservation</h1>
              <h2 className="text-2xl text-white">{setClientStatistic.deletedReservation}</h2>
            </div>
          </div>
...
export default function AdminDashboard() {
   ...
   useEffect(()=>{
    ...
    const fetchData =  async () => {
      try{
        const response = await fetch("http://localhost/hotel-website/Back-End/ReservationStatistics.php" ,{
          method:'GET',
          headers :{
    ...
    <>
      <div className="relative mt-[20em] p-8 bg-slate-900 h-screen">
        ...
        <div className="flex justify-between">
          ...
          <div ref={ref}>
            {activeTab == "Dashboard" && <DisplayClientStatistics setClientStatistic={CounterEffect}/>}
